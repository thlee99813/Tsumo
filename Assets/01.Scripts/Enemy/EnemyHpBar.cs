using DG.Tweening;
using UnityEngine;
using UnityEngine.UI;

[DisallowMultipleComponent]
public class EnemyHpBar : MonoBehaviour
{
    [Header("Reference")]
    [SerializeField] private IngameController _ingameController;
    [SerializeField] private Slider _slider;
    [SerializeField] private CanvasGroup _canvasGroup;

    [Header("Follow")]
    [SerializeField] private Vector3 _worldOffset = new Vector3(0f, 2f, 0f);
    [SerializeField] private bool _hideWhenOffScreen = true;

    [Header("Tween - Value")]
    [SerializeField] private float _decreaseDuration = 0.2f;
    [SerializeField] private float _increaseDuration = 0.12f;

    [Header("Tween - Punch")]
    [SerializeField] private Vector3 _decreasePunch = new Vector3(0.12f, 0.12f, 0f);
    [SerializeField] private float _decreasePunchDuration = 0.2f;
    [SerializeField] private int _decreasePunchVibrato = 8;
    [SerializeField] private float _decreasePunchElasticity = 0.9f;

    private RectTransform _rectTransform;
    private Enemy _enemy;
    private Tween _valueTween;
    private Tween _punchTween;
    private int _lastHp;
    private bool _isEnemyBound;
    private bool _isControllerBound;
    private bool _isVisibleByState;

    private void Reset()
    {
        _rectTransform = transform as RectTransform;
        _slider = GetComponent<Slider>();
        _canvasGroup = GetComponent<CanvasGroup>();
    }

    private void Awake()
    {
        CacheReferences();
    }

    private void OnEnable()
    {
        CacheReferences();
        BindController();
        RebindEnemy(_ingameController != null ? _ingameController.CurrentEnemy : null);
        HideImmediate();
    }

    private void OnDisable()
    {
        UnbindController();
        UnbindEnemy();
        KillTweens();
    }

    private void OnDestroy()
    {
        KillTweens();
    }

    private void LateUpdate()
    {
        UpdateFollow();
    }

    // 필요한 참조를 캐시한다.
    private void CacheReferences()
    {
        if (_rectTransform == null)
            _rectTransform = transform as RectTransform;

        if (_slider == null)
            _slider = GetComponent<Slider>();

        if (_canvasGroup == null)
            _canvasGroup = GetComponent<CanvasGroup>();
    }

    // IngameController 이벤트를 구독한다.
    private void BindController()
    {
        if (_ingameController == null || _isControllerBound)
            return;

        _ingameController.OnEnemyChanged += HandleEnemyChanged;
        _ingameController.OnEnemyAppear += HandleEnemyAppear;
        _isControllerBound = true;
    }

    // IngameController 이벤트 구독을 해제한다.
    private void UnbindController()
    {
        if (_ingameController == null || !_isControllerBound)
            return;

        _ingameController.OnEnemyChanged -= HandleEnemyChanged;
        _ingameController.OnEnemyAppear -= HandleEnemyAppear;
        _isControllerBound = false;
    }

    // 현재 추적 중인 Enemy 이벤트를 구독한다.
    private void BindEnemy()
    {
        if (_enemy == null || _isEnemyBound)
            return;

        _enemy.OnHpChanged += HandleHpChanged;
        _enemy.OnEnemyDead += HandleEnemyDead;
        _isEnemyBound = true;
    }

    // 현재 추적 중인 Enemy 이벤트 구독을 해제한다.
    private void UnbindEnemy()
    {
        if (_enemy == null || !_isEnemyBound)
            return;

        _enemy.OnHpChanged -= HandleHpChanged;
        _enemy.OnEnemyDead -= HandleEnemyDead;
        _isEnemyBound = false;
    }

    // Enemy 타겟을 다시 바인딩한다.
    private void RebindEnemy(Enemy enemy)
    {
        UnbindEnemy();
        _enemy = enemy;
        BindEnemy();

        if (_enemy != null)
        {
            RefreshBarImmediate();
        }
    }

    // 적이 바뀌었을 때 타겟을 교체하고 일단 숨긴다.
    private void HandleEnemyChanged(Enemy enemy)
    {
        RebindEnemy(enemy);
        HideImmediate();
    }

    // 적 등장 연출 시점에 HP 바를 다시 보여준다.
    private void HandleEnemyAppear()
    {
        if (_enemy == null || _enemy.IsDead)
            return;

        RefreshBarImmediate();
        SetVisibleState(true);
    }

    // 현재 Enemy 상태를 기준으로 HP 바를 즉시 동기화한다.
    private void RefreshBarImmediate()
    {
        if (_enemy == null || _slider == null)
            return;

        KillTweens();

        _slider.minValue = 0f;
        _slider.maxValue = _enemy.MaxHp;
        _slider.value = _enemy.CurrentHp;
        _lastHp = _enemy.CurrentHp;

        transform.localScale = Vector3.one;
    }

    // 적 체력이 변했을 때 슬라이더 값을 갱신한다.
    private void HandleHpChanged(int currentHp)
    {
        if (_enemy == null || _slider == null)
            return;

        if (!Mathf.Approximately(_slider.maxValue, _enemy.MaxHp))
            _slider.maxValue = _enemy.MaxHp;

        bool isDecrease = currentHp < _lastHp;
        _lastHp = currentHp;

        _valueTween?.Kill();

        if (isDecrease)
        {
            _valueTween = _slider
                .DOValue(currentHp, _decreaseDuration)
                .SetEase(Ease.OutCubic);

            PlayDecreasePunch();
        }
        else
        {
            _valueTween = _slider
                .DOValue(currentHp, _increaseDuration)
                .SetEase(Ease.OutQuad);
        }
    }

    // 적이 죽으면 HP 바를 숨긴다.
    private void HandleEnemyDead()
    {
        HideImmediate();
    }

    // 적 월드 좌표를 화면 좌표로 변환해 HP 바 위치를 갱신한다.
    private void UpdateFollow()
    {
        if (_enemy == null || _rectTransform == null || Camera.main == null)
            return;

        Vector3 screenPosition = Camera.main.WorldToScreenPoint(_enemy.transform.position + _worldOffset);

        bool isVisible =
            screenPosition.z > 0f &&
            screenPosition.x >= 0f &&
            screenPosition.x <= Screen.width &&
            screenPosition.y >= 0f &&
            screenPosition.y <= Screen.height;

        if (_canvasGroup != null)
        {
            if (!_isVisibleByState)
            {
                _canvasGroup.alpha = 0f;
            }
            else if (_hideWhenOffScreen)
            {
                _canvasGroup.alpha = isVisible ? 1f : 0f;
            }
            else
            {
                _canvasGroup.alpha = 1f;
            }
        }

        if (!isVisible)
            return;

        _rectTransform.position = screenPosition;
    }

    // 체력이 줄어들 때 HP 바에 펀치 모션을 준다.
    private void PlayDecreasePunch()
    {
        _punchTween?.Kill();

        transform.localScale = Vector3.one;

        _punchTween = transform.DOPunchScale(
            _decreasePunch,
            _decreasePunchDuration,
            _decreasePunchVibrato,
            _decreasePunchElasticity);
    }

    // HP 바를 즉시 숨긴다.
    private void HideImmediate()
    {
        SetVisibleState(false);
        KillTweens();

        if (_canvasGroup != null)
            _canvasGroup.alpha = 0f;
    }

    // 상태 기반 가시성을 설정한다.
    private void SetVisibleState(bool isVisible)
    {
        _isVisibleByState = isVisible;
    }

    // 진행 중인 트윈을 정리한다.
    private void KillTweens()
    {
        _valueTween?.Kill();
        _valueTween = null;

        _punchTween?.Kill();
        _punchTween = null;
    }
}